...
export default function RecetteDeGrandMere() {
  ...
  useEffect(() => {
    const loadMessages = async () => {
      try {
        const response = await fetch(`/api/messages?pageTitle=recette`);
        if (response.ok) {
          ...
          if (data.messages && data.messages.length > 0) {
            ...
            const welcomeMessage = {
              content: `Bienvenue dans le chat des recettes de grand mère, Je suis votre nouvelle grand-mère prête a tout pour régler le moindre de vos petit problèmes de santé ! Attention il est possible que certains de mes conseilles soient erroné, je perds un peu la tête en ce moment...`,
              sender: "bot",
            };
