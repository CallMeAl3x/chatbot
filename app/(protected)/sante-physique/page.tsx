...
import Brain3D from "@/app/components/Brain3d";
...
export default function SantePhysiquePage() {
  ...
  useEffect(() => {
    const loadMessages = async () => {
      try {
        const response = await fetch(`/api/messages?pageTitle=sante`);
        if (response.ok) {
          ...
          if (data.messages && data.messages.length > 0) {
            ...
            const welcomeMessage = {
              content: `Bienvenue dans le chat santé physique. Je suis votre assistant virtuel, prêt à vous aider et à répondre à vos questions. Attention : je ne remplace en aucun cas un médecin ou un professionnel de santé physique. Pour tout problème médical sérieux, veuillez consulter un professionnel. Comment puis-je vous assister aujourd'hui ?`,
              sender: "bot",
            };
